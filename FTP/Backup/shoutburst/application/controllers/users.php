...
class Users extends CI_Controller {
	...
	public function index()
	{
		...
		/*For Actual Tags*/
		$data['actualtagmap'] = array();
		foreach ($data['users'] as $u) {
			$user_id = $u->user_id;
			$tags_list = $this->users_model->my_actual_tag($user_id);
			//$tags_list = implode(",",$tags_list);
			$data['actualtagmap'][$user_id] = $tags_list;
		}		
	...
	public function add()
	{
		...
		# Get all actual tags
		$data['actual_tags'] = $this->tags->get_actual_tags($comp_id);


        # Get all group Tags
        $data['group_tags'] = $this->tags->get_group_tags($comp_id);

		$data['access_level']=$this->session->userdata['access'];
		...
		if (isset($post) && !empty($post))
		{
			...
			$group_tags	= 	isset($post['group_tags']) ? $post['group_tags'] : array();
			$user_pin	=	rtrim($post['user_pin']);
			$user_email	=	rtrim($post['user_email']);
			$actual_tags = $post['actual_tags'];
			$user_photo = "";
						
			if ($accessArr == COMP_ADMIN){
				$user_pin = 0;
			}
					...
					if ( empty($return) )
					{
						...
						if( empty ( $ret ) )
						{
							...
							{							
								...
								$insert_agent['password'] = md5($post['password']);
								...
								if( $insert_id > 0 )
								{
									...
									# add group_tags
									$this->users_model->add_group_tags($comp_id, $insert_id, $group_tags);
									
									# add user actual team tags
									$this->users_model->assign_actual_team_tags($comp_id,$insert_id,$actual_tags);


									$this->users_model->addUserComp( $insert_id , $accessArr , $comp_id , $user_pin , 0 );
									
						//			if ( $this->send_email($user_name,$user_login,$user_email,$insert_agent['security_code']) )
						//			{
						...
						else
						{
							...
							$this->session->set_flashdata('message', '<div id="message" class="error">Email already exists.</div>');						
						}
					}//end if(empty($return))
					else
					{
	...
	public function edit()
	{
			...
			if( $user_name!='' && $user_login!=''  &&  $user_email!='' && !empty($accessArr))
			{
				...
				if ( !$result ){
					...
					if ( !empty($return) )
					{
						...
						$update_uc["transcriber"]=isset($transcribe) ? $transcribe : "0";
						$this->db->where('user_id', $user_id);
						$this->db->where('comp_id', $comp_id);
						$this->db->update('user_companies', $update_uc);
						
						
						# remove existing tags + group_tags
						$this->users_model->remove_tags($comp_id, $user_id);
						$this->users_model->remove_group_tags($comp_id, $user_id);
						
						# add user_tags + group_tags
						$this->users_model->add_tags($comp_id, $user_id, $tags);
						$this->users_model->add_group_tags($comp_id, $user_id, $group_tags);
						

						# Get user assigned group tags
						$user_actual_tags = $this->users_model->my_actual_tag($user_id);
						# add user actual team tags
						if($user_actual_tags != 'No Tag Assigned')
							$this->users_model->update_actual_tags($comp_id,$user_id,$actual_tag[0]);
						else
							$this->users_model->assign_actual_team_tags($comp_id,$user_id,$actual_tag);
		...
		if ( is_numeric($user_id) ) 
		{
			...
			# Get user assigned actual tags
			$data['user_actual_tags'] = $this->users_model->my_actual_tag($user_id);
			# Get user assigned group tags
			$data['user_group_tags'] = $this->users_model->my_group_tags($user_id);
	 ...
	 * @name:	send_email
	 */	
	public function send_email( $name='sajid',$user_login="sajid", $to='sajid@nextgeni.com', $security_code='786' )
	{
		...
		$message = "Hi ".ucwords($name).",<br /><br />Your account has been created.<br /><br /> Your Username is: " . $user_login .  ".<br /> <br />Please set your password by clicking the following link <br /><br />".$link;
