...
class Company_model extends CI_Model {
	...
	{	
    	...
    	$insert_uc['acc_id'] = COMP_ADMIN;    
    	$insert_uc["transcriber"]=$data['transcribe'];
    		
    	$this->db->insert('user_companies', $insert_uc);  

	# NX : insert into dash as well
	$insert_dashboard['user_id'] = $user_id;
        $insert_dashboard['comp_id'] = $comp_id;
        $insert_dashboard['qdr_1'] = 0;
        $insert_dashboard['qdr_2'] = 0;
        $insert_dashboard['qdr_3'] = 0;
        $insert_dashboard['qdr_4'] = 0;
	...
	function getCompanyPins($comp_id) {
		$sql = "select user_pin from user_companies where status=1 and comp_id='$comp_id'";
		$pins = $this->db->query($sql);
	...
	{		
		$data = array(
				...
				'last_change'=> date("Y-m-d H:i:s",time())
				);
		if(isset($post['logo']))
		{
			$data['logo']=$post['logo'];
		...
		$this->db->where('comp_id', $comp_id);
		$chk_targetsetup = $this->db->get('target_setup')->result();
		...
		$data = array(
						...
						"highest_score"=>$post['highest_score']
						);
		
		if(empty($chk_targetsetup))
		{
			$data['comp_id'] = $comp_id;
			$this->db->insert('target_setup', $data);
		}
		else
		{
			$this->db->where('comp_id', $comp_id);
