...
HTML;
								...
								for($i=2; $i<$totalColumn; $i++){
									...
									if($totalSurveyColPosition!==false && $totalSurveyColPosition==$i && $totalSurveyValDisplay==0)
									{
										...
										if ( $selectedColoumnHeadingArr[$count] == 'Average Score' || 
											 $selectedColoumnHeadingArr[$count] == 'Total Score' ){
											$coloumnVal					=	number_format( floatval($agentRecordDateWiseRow[$j]) );
										} else {
									...
									if($selectedColoumnHeadingArr[$count]=='Recording' && strtolower($report_interval) != 'live'){
										$urlLink 	= base_url().'reports/recording_play/';
										$coloumnVal	= "<a href='#myModal' id='modal1' style='font-size: 20px; text-decoration: none;' class='glyphicon glyphicon-play-circle modal-iframe' role='button' data-toggle='modal' data-filename='$coloumnVal' data-src='$urlLink' data-height='300' data-width='500'></a>";	
									}
...
HTML;
								...
								if($subTotalLoop>0){
									for($i=0;$i<$subTotalLoop;$i++){
										...
										$overAllTotalArr[$i][]		= is_numeric($subTotalVal) ? number_format($subTotalVal) : $subTotalVal;
...
HTML;
//bind modal all report interval except live
if(strtolower($report_interval) != 'live'){
$html .= <<<HTML
			...
			jQuery('div#myModal').live('hide.bs.modal', function () {
				jQuery(".modal-body#jquery_jplayer_1").jPlayer("stop");
			   	jQuery('.modal-body').html('Please Wait...');           
			});
			jQuery('div#myModal').live('hidden.bs.modal', function () {
				jQuery(".modal-body#jquery_jplayer_1").jPlayer("stop");
			  	jQuery('.modal-body').html('Please Wait...');           
			});		
		</script>
