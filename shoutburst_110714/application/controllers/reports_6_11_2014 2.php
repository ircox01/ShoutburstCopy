		...
		//get chart coloumn result text on basis of selection 
			switch ($report_period)
			{
			    ...
			    case "last month":
			        ...
			        break;
			    case "custom":
			    	$chartColoumn[]	= "CONCAT('Record Shown from ','$start_date' ,' to ' , '".$post['end_date']."') AS recordShownMessage";
			        break;
...
//		exit();
	...
	function print_csv()
	{
		...
		$report_name = $result[0]['report_name']."_".date("d-m-Y_H-i",time());		
		...
		if(!empty($data))
		{
			...
			foreach ($data AS $row)
			{
				...
				header( "Content-disposition: filename=".$report_name.".csv");
	...
	{		
		...
		$result = $this->db->query("SELECT report_name, report_query,columns_name,report_period,report_interval FROM reports WHERE report_id=".$report_id )->result_array();
