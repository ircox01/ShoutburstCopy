				...
				?>
					...
					<li id="<?php echo $dashboard['qdr_'.$d]; ?>" report_id="<?php echo $report_id[$d]?>" class="col-md-6">
						...
						<?php if (!empty($report_id[$d]) && ($report_id[$d] > 0)){?>
					  	<h3 class="title"></h3>
						<a href="#myModal" id='modal1' class="modal-iframe" role="button" data-toggle="modal" data-src="<?php echo base_url().'reports/view_report/'.$report_id[$d].'/d'?>" data-height=500 data-width=100% ></a>
						<?php if ( ($is_dashboard[$d] == 1) && ($privacy[$d] == 'global') ){?>
						<iframe scrolling="no" width="100%" height="90%" style="border: 0px;" 
							src="<?php echo base_url().'reports/view_report/'.$report_id[$d].'/d'?>"></iframe>
						<?php }?>
                                                <?php } else { echo "<br /><br /><br /><br /><br /><br /><br /><br /><br /><div style='text-align:center;'><img src='/shoutburst/img/logo_withbg.png' /></div>"; } ?>
					  </div>
					</li>
					<?php }else{?>
