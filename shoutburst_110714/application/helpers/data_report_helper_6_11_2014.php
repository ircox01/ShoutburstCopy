		...
		if(!empty($post)){
			...
			switch ($report_period) {
			    ...
			    case "custom":
			    	...
			    	$start_date = str_replace('/', '-', $post['start_date']);
					$start_date		= date('Y-m-d H:i',strtotime($start_date));//$post['start_date'];// 01/05/2014 18:20 date('d/m/Y H:i',strtotime($post['start_date']));
					$end_date = str_replace('/', '-', $post['end_date']);
					$end_date		=  date('Y-m-d H:i',strtotime($end_date));//$post['end_date'];//  date('d/m/Y H:i',strtotime($post['end_date']));
					
			        $whereCondition =  " $dateTime >= '$start_date' AND $dateTime <= '$end_date' ";
			        break;    
...
//				}
	...
	function getDataColumn($post,$select)
	{
		...
		if(!empty($post)){
			...
			$start_date = str_replace('/', '-', $post['start_date']);
			$start_date		= date('d-m-Y H:i',strtotime($start_date));//$post['start_date'];// 01/05/2014 18:20 date('d/m/Y H:i',strtotime($post['start_date']));
			$end_date = str_replace('/', '-', $post['end_date']);
			$end_date		=  date('d-m-Y H:i',strtotime($end_date));//$post['end_date'];//  date('d/m/Y H:i',strtotime($post['end_date']));
			if(in_array($report_period,array('current hour','day','today','yesterday','current week','last week','current month','last month','custom'))){
				$dateTime = 's.date_time';
			}
