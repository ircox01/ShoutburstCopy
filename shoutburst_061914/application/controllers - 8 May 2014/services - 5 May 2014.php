			...
			case 'question':
				...
				{					        		
			        ...
			        $score = 'q' . addslashes($question_number) . '=' . addslashes($_GET['answer']);			       	
			       	$result= $this->service_model->updateSurveyScore($score, $total_score , $average_score , $_GET['action'] , $callid);			
			        echo $output = "<container>\n<ok>1</ok>\n</container>\n";
				} else {
					echo $output = "<container>\n<ok>0</ok>\n</container>\n";
				}
...
/*				
				...
				$curl = curl_init();
				curl_setopt($curl, CURLOPT_URL, 'ftp://@'.$ftp_server.$filepath);
				...
				if($ftp_connect)
				{
					...
					if(!empty($file_list))
					{
						...
						{ 
							echo $file."<br>";
							if($file!="." || $file!=".." || $file!=""){
								if(preg_match("/^({$tempfilename}.*.)/i" , $file)){
								//if($time== $fileTime)
								//{
									$filename = $file;
									$found = true;
						...
						if ($found == true){
							$this->service_model->updateSurveyRecording($filename, $filepath ,$_GET['action'], $_GET['callid']);
							echo $output = "<container>\n<ok>".$filename."</ok>\n</container>\n";
						} else {
							$this->service_model->updateSurveyRecording("not found", $filepath ,$_GET['action'], $_GET['callid']);
							echo $output = "<container>\n<ok>0</ok>\n</container>\n";
						}
