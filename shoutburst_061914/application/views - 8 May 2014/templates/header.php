...
<?php if ( ($this->uri->segment(2) != 'launch') && ($this->uri->segment(2) != 'congrats') ){?>
	...
	<script type="text/javascript">
		$(document).ready( function () {
			...
			$('#dashboardDatatable').dataTable({"aaSorting": [[ 2, "desc" ]]}); 
		} );
	</script>
	<!--// Usman's Datatable -->
	...
	<link href="<?php echo base_url(); ?>css/jquery-validator.css?v=11" rel="stylesheet" type="text/css" />
	...
	<!-- jQuery Chosen -->
	<link href="<?php echo base_url(); ?>css/chosen.css?v=11" rel="stylesheet" type="text/css" />
	...
	<div id="main-nav">
		...
		switch ($access_level){
			...
			case COMP_ADMIN:
				...
				echo anchor("tags","<span class='icon icon-teams'></span>Teams", array('class'=>($segment=='tags'?'active':'')));
				echo anchor("reports","<span class='icon icon-reports'></span>Reports", array('class'=>($segment=='reports'?'active':'')));
				echo anchor("wallboard","<span class='icon icon-wallboards'></span>Wallboard", array('class'=>($segment=='wallboard'?'active':'')));
