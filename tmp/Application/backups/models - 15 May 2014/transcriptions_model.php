...
class Transcriptions_model extends CI_Model {
	 ...
	 */
	public function insert_transcription($insert_transcription)
	{
	...
	function get_transcriptions($comp_id, $transcriber)
	{
		$query = array();
		if ( $transcriber == 1){
			$sql = "SELECT s.*, t.transcription_id, t.transcriptions_text, t.sentiment_score, t.gender 
					FROM surveys s
					LEFT JOIN transcriptions t ON s.sur_id = t.sur_id
					WHERE s.comp_id = $comp_id AND s.processed = 1 AND downloaded=1";
