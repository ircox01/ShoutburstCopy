...
class Reports extends CI_Controller
{
	...
	public function query_builder()
	{
		...
		$data = $this->session->userdata('comp_id');
		$where =  $where .' AND  uc.comp_id = ' .$data;
		...
		{
			//set where condition basis of chart selection yearly, monthly
			if($the_transcription_add==1)
				$where .= " AND t.empty!=1 AND t.background_comments!=1 ";
		...
		if( $report_type == 'detail' )
		{
			...
			// NX
			$select = "DATE_FORMAT(date_time,'%Y-%m-%d  %H:%i:%s') as date_time,".$select;
		}		
		...
		if ($report_type == 'word cloud') {
			...
			if ($where == "") {
				$where .= "transcriptions_text IS NOT NULL AND transcriptions_text != ''";
			} else {
				$where .= " AND transcriptions_text IS NOT NULL AND transcriptions_text != ''";
			}
			$where  = $where . " AND empty != 1 AND background_comments != 1 ";
			$dateTime = "s.date_time";

            switch ($post['report_period']) {
                case "current hour":
                    $where .= 'AND TIMEDIFF(DATE_FORMAT(s.date_time, "%Y-%m-%d %H:%i"),DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 HOUR), "%Y-%m-%d %H:%i"))>=0 ';
                    break;
                case "day":
                    $custom_date    =   date('Y-m-d',strtotime($post['custom_date']));
                    $where .=  "AND DATE($dateTime) = '$custom_date' ";
                    break;
                case "today":
                    $where .=  "AND DATE($dateTime) = CURRENT_DATE()";
                    break;
                case "yesterday":
                    $where .=  "AND DATE($dateTime) = DATE(DATE_SUB(NOW(), INTERVAL 1 DAY))";
                    break;
                case "current week":
                    $where .=  "AND DATE($dateTime) >= DATE(DATE_SUB(NOW(), INTERVAL 1 WEEK))";
                    break;
                case "last week":
                    $where .=  "AND DATE($dateTime) >= DATE(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AND DATE($dateTime) <= DATE(DATE_SUB(NOW(), INTERVAL 1 WEEK))";
                    break;
                case "current month":
                ...
                case "custom":
                    //format start and end time for sql query
                            $start_date             = date('Y-m-d H:i',strtotime($post['start_date'])); //$post['start_date'];// date('Y-m-d H:i',strtotime($post['start_date']));
                            $end_date               = date('Y-m-d H:i',strtotime($post['end_date']));//$post['end_date'];//  date('Y-m-d H:i',strtotime($post['end_date']));

                   // $whereCondition =  "DATE_FORMAT($dateTime,'%%Y-%m-%d %H:%i') >= '$start_date' AND DATE_FORMAT($dateTime,'%%Y-%m-%d %H:%i') <= '$end_date' ";
                            $where .=  "AND s.date_time BETWEEN CAST('$start_date' as date) AND CAST('$end_date' as date)";                           
                    break;
            }
	    ...
	    */
		...
		if ($report_type == 'word cloud') {
			...
			if ($text != "") {
				$wciframe = "<iframe src='".base_url()."wordcloud_txt.php?text=$text64&logo=$logo' height=600 width=1100 frameBorder='0' style='overflow:hidden;'/>";
				echo $wciframe;
			} else {
				echo "<span style='text-align:center'><b>No word cloud data to display</b></span>";
			}
...
public function wcDraw($query) {
		...
		{			
			...
			{				
				...
				switch ($post['filter'][$i])
				{
					...
					case 'e':
						if($post['data_type'][$i] ==="tag_name" ||$post['data_type'][$i] ==="user_pin" || $post['data_type'][$i] ==="cli" || $post['data_type'][$i] ==="dialnumb" || $post['data_type'][$i] ==="full_name")
						{
							$operator = "IN";
						}
						else
					...
					case 'ne':
						if($post['data_type'][$i] ==="tag_name"|| $post['data_type'][$i] ==="user_pin" || $post['data_type'][$i] ==="cli" || $post['data_type'][$i] ==="dialnumb")
						{
							$operator = "NOT IN";
						}else
							$operator = '!=';
	...
	function getChartColumn($post){
		...
		if(!empty($post)){
			...
			$report_period		=	$post['report_period'];
			$report_interval	=	$post['report_interval'];
			$report_fields = explode(",",strtolower($post['reports_fields']));
			...
			switch ($report_interval) 
			{
			    case "live":
			    	...
			    	$chartColoumn[]	= "AVG($y_axis_label) AS totalSurveyScore";
					$chartColoumn[] = "DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(date_time) - MOD(UNIX_TIMESTAMP(date_time), $liveInterval)), '%Y-%m-%d %H:%i') AS reportDate";
					$chartColoumn[] = "GROUP_CONCAT(CONCAT($y_axis_label,'---+++--', Date_format(DATE_SUB(date_time, INTERVAL 5 MINUTE),'%Y-%m-%d %H:%i')) SEPARATOR '___---___') AS totalWithRange";
			        break;
			    case "minutes":
			    	$chartColoumn[]	= "AVG($y_axis_label) AS totalSurveyScore";
					$chartColoumn[] = "DATE_FORMAT(FROM_UNIXTIME(UNIX_TIMESTAMP(date_time) - MOD(UNIX_TIMESTAMP(date_time), 300)), '%Y-%m-%d %H:%i') AS reportDate";
					$chartColoumn[] = "GROUP_CONCAT(CONCAT($y_axis_label,'---+++--', Date_format(date_time, '%Y-%m-%d %H:%i')) SEPARATOR '___---___') AS totalWithRange";
			        break;
			    case "hours":
			    	$chartColoumn[]	= "AVG($y_axis_label) AS totalSurveyScore";
		...
		//get chart coloumn result text on basis of selection 
			...
			{
			    case "current hour":
					$chartColoumn[]	= "CONCAT('Record Shown from ',DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 HOUR),'%Y-%m-%d %H:%i'),' to ' , DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 0 HOUR),'%Y-%m-%d %H:%i')) AS recordShownMessage";
			        break;
			    case "day":
			    	$custom_date	=  $post['custom_date'];
			    	$chartColoumn[]	= "CONCAT('Record Shown for ', '$custom_date') AS recordShownMessage";
			        break;
			    case "today":
			    ...
			    case "last month":
			        ...
			        break;
			    case "custom":
			    	$chartColoumn[]	= "CONCAT('Record Shown from ',DATE_FORMAT('$start_date','%Y-%m-%d %H:%i') ,' to ' , DATE_FORMAT('".$end_date."','%Y-%m-%d %H:%i')) AS recordShownMessage";
			        break;
	...
	function chartWhereCondition($post){
		...
		if(!empty($post))
		{
			...
			$fields = explode(",",strtolower($post['reports_fields']));
			...
			switch ($report_period) {
			    ...
			    case "custom":
			    	...
			    	//format start and end time for sql query
					$start_date		=  date('Y-m-d H:i',strtotime($post['start_date']));
					$end_date		=  date('Y-m-d H:i',strtotime($post['end_date']));//  date('Y-m-d H:i',strtotime($post['end_date']));
				...
				if(!empty($dataArr)){
						...
						if($report_type=='line graph'){
							...
							foreach($dataArr as $dataRow){
									...
									$SingleRangeElements = explode('___---___',$dataRow['totalWithRange']);
									$SingleUserData=array();
									
									foreach($SingleRangeElements as $SingleRangeElement){
										$UserDataElem=explode('---+++--', $SingleRangeElement);
										//$UserDataList[] = $UserDataElem;
										if (!in_array($UserDataElem[1], $graph_xAxix)) {
											$graph_xAxix[]=  $UserDataElem[1];
										}

										if(isset($SingleUserData[$UserDataElem[1]]) && !empty($SingleUserData[$UserDataElem[1]])){
											$SingleUserData[$UserDataElem[1]][0] += $UserDataElem[0];
											$SingleUserData[$UserDataElem[1]][1] += 1 ;
										}
										else{
											
											$SingleUserData[$UserDataElem[1]]=array();
											$SingleUserData[$UserDataElem[1]][0] = $UserDataElem[0];
											$SingleUserData[$UserDataElem[1]][1] = 1;	
										}
								...
								$SingleUserDataArray=array();
								foreach($SingleUserData as $x=>$SingleUData){
									$SingleUserDataArray[$x][0]=$SingleUData[0]/$SingleUData[1];
								}
								$chartDataArr[$dataRow['AgentFullName']]= $SingleUserDataArray;
...
//		exit();
		...
		{			
			...
			if (!empty($report_id) && is_numeric($report_id))
			{
				...
				if (!empty($report))
				{
					...
					if ($this->uri->segment(4)) { // Dashboard
						$this->load->view('reports/view_report1', $data);
					} else { // Full graphs from dashboard and reports page 
	...
	function print_csv()
	{
		...
		$output .= "\n";
		$output .= "Report Date : ".date('Y-m-d',strtotime(date('c')));
		$output .= "\n";
