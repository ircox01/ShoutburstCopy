...
class Reports extends CI_Controller {
	...
	public function query_builder()
	{
		...
		if(!empty($post['reports_fields']))
		{
			...
			{				
				...
				else if( ( $k== "q1_score" || $k== "q2_score" || $k== "q3_score" || $k== "q4_score" ||$k== "q5_score" ) && $report_type=="data")
				{
					...
					$cols[$k]= "avg(total_score) as total_score";
				}
				if ($cols[$k] != "") {
					$fields[] = $cols[$k];
				}
		...
		if( $report_type == 'detail' )
		{
			...
			// NX
			$select = "DATE_FORMAT(date_time,'%d/%m/%Y   %H:%i:%s') as date_time,".$select;
		}		
...
//$wciframe = "<iframe src='/shoutburst/wordcloud_txt.php?text=$text64&logo=$logo' height=600 width=900 frameBorder='0' style='overflow:hidden;'/>";
if ($text != "") {
	...
	echo "<span style='text-align:center'><b>No word cloud data to display</b></span>";
		...
		{			
			...
			//$select = "IFNULL(full_name,'Not Found') AS full_name,IFNULL(user_pin,'Not Found') AS user_pin,q1,q2,q3,average_score,recording,'',full_name AS agentName , cc.logo AS logo";
		...
		//get chart coloumn result text on basis of selection 
			switch ($report_period)
			{
			    ...
			    case "last month":
			        ...
			        break;
			    case "custom":
			    	$chartColoumn[]	= "CONCAT('Record Shown from ',DATE_FORMAT('$start_date','%d/%m/%Y %H:%i') ,' to ' , DATE_FORMAT('".$end_date."','%d/%m/%Y %H:%i')) AS recordShownMessage";
			        break;
			...
			if(in_array('campaign', $report_fields)){
				$chartColoumn[]	= 'GROUP_CONCAT(DISTINCT(camp_name)) AS AgentFullName';
		...
		if(!empty($post))
		{
			...
			switch ($report_period) {
			    ...
			    case "custom":
			    	...
			    	//format start and end time for sql query
					$start_date		=  date('Y-d-m H:i',strtotime($post['start_date']));
					$end_date		=  date('Y-d-m H:i',strtotime($post['end_date']));//  date('Y-m-d H:i',strtotime($post['end_date']));
