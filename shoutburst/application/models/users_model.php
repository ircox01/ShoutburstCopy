...
class Users_model extends CI_Model {
    ...
    public function getUsers( $comp_id="" )
    {
    	...
    	$data = $this->db->query("SELECT u.user_id,u.status,u.full_name,u.user_name,u.email, uc.user_pin, GROUP_CONCAT(uc.acc_id) AS acc_id
    							...
    							WHERE uc.comp_id={$comp_id} AND uc.acc_id >".SUPER_ADMIN." AND u.user_id!=".$user_id." AND u.status = 1 GROUP BY u.user_id ")->result();
    	return $data;
    }
    ...
    /*
     * @author: Usama Noman
     * @name:   assign_actual_team_tags
     */
    public function assign_actual_team_tags($comp_id, $user_id, $tags)
    {
        if (!empty($tags))
        {
            $batch = array();
            ...
            foreach ($tags as $k)
            {
                ...
                $batch[$i]['actual_tag_id'] = $k;
                $i++;
            }
            //die($batch);
            $this->db->insert_batch('user_actual_tags', $batch);
        }
    }
    ...
    public function add_group_tags($comp_id, $user_id, $group_tags)
    {
    	if (!empty($group_tags))
    	{
    		$batch = array();
    		$i = 0;
    		foreach ($group_tags as $k)
    		{
    			$batch[$i]['comp_id'] = $comp_id;
    			$batch[$i]['user_id'] = $user_id;
    			$batch[$i]['tg_id'] = $k;
    		...
    		$this->db->insert_batch('user_group_tags', $batch);
    	}
    }
    ...
    public function remove_group_tags($comp_id, $user_id)
    {
        $this->db->where('comp_id', $comp_id);
        $this->db->where('user_id', $user_id);
                $this->db->delete('user_group_tags');
    }
     ...
     */
    /*public function  my_actual_tag($user_id)
    {
        $tags = array();
        $my_tags = $this->db->query("SELECT actual_tag_id FROM user_actual_tags WHERE user_id = {$user_id}")->result_array();
        foreach ($my_tags as $k){
            $tags[] = $k['actual_tag_id'];
        }
        return $tags[0];
    }*/
    ...
    /*
     * @author: Usama Noman
     * @name:   update_actual_tags($comp_id,$user_id,$actual_tag)
     */
    public function update_actual_tags($comp_id,$user_id,$actual_tag){
            $update_data['actual_tag_id'] = $actual_tag;
            $update_data['comp_id'] = $comp_id;
            $update_data['user_id'] = $user_id;
            
            $this->db->where('user_id', $update_data['user_id']);
            $this->db->update('user_actual_tags', $update_data);
    }
    ...
    public function my_group_tags($user_id)
    {
        $tags = array();
        $my_tags = $this->db->query("SELECT tg_id FROM user_group_tags WHERE user_id = {$user_id}")->result_array();
        foreach ($my_tags as $k){
                $tags[] = $k['tg_id'];
        }
        return $tags;
    ...
    {
        $tags = array();
        $my_tags = $this->db->query("SELECT actual_tag_name FROM user_actual_tags INNER JOIN actual_tags on actual_tags.actual_tag_id = user_actual_tags.actual_tag_id WHERE status=1 and user_actual_tags.user_id = {$user_id}")->result_array();
        foreach ($my_tags as $k){
            $tags[] = $k['actual_tag_name'];
        }
     ...
     * @name:	my_tags($user_id)
     */
    public function my_tags_list($user_id)
    {
    	$tags = array();
    	$my_tags = $this->db->query("SELECT tag_name FROM user_tags INNER JOIN tags on tags.tag_id = user_tags.tag_id WHERE status=1 and user_tags.user_id = {$user_id}")->result_array();
    	foreach ($my_tags as $k){
    		$tags[] = $k['tag_name'];
    	}
    	return $tags;
    ...
    public function my_group_tags_list($user_id)
    {
    	$tags = array();
    	$my_tags = $this->db->query("SELECT tg_name FROM user_group_tags INNER JOIN tags_group on tags_group.tg_id = user_group_tags.tg_id WHERE status=1 and user_group_tags.user_id = {$user_id}")->result_array();
    	foreach ($my_tags as $k){
    		$tags[] = $k['tg_name'];
    	}
    	return $tags;
    ...
    public function resetPassword( $data=array() )
    {
    	...
    	$query = $this->db->query("SELECT user_id FROM users WHERE user_id='".$data['user_id']."' AND password='".md5($data['epassword'])."'");
