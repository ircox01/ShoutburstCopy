...
class Transcriptions_model extends CI_Model {
	...
	public function update_transcription($update_transcription)
	{
        ...
        public function next_transcription($comp_id, $transcriber, $current_transcription)
        {
                $query = array();
		if ( $transcriber == 1)
		{
			$sql = "SELECT t.transcription_id
					FROM surveys s
					LEFT JOIN transcriptions t ON s.sur_id = t.sur_id
					WHERE s.comp_id = $comp_id AND s.processed = 1 AND downloaded=1 AND (LENGTH(transcriptions_text) < 5 OR transcriptions_text IS NULL) AND t.transcription_id > $current_transcription";
			$query = $this->db->query($sql)->result();			
                }
                return $query;
        }
