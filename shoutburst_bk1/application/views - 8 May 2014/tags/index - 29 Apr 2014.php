						...
						<?php if (!empty($tags)){?>
							...
							<?php foreach ($tags as $k){?>
								<li class="team" tag_id="<?php echo $k->tag_id?>" style="background-color: <?php echo $k->color?>;">
									<a class="team-href" href="<?php echo base_url().'tags/edit/tags/'.$k->tag_id?>"><?php echo $k->tag_name?></a>
					...
					<div class="tag-store">
						...
						<?php if (!empty($tags_group)){?>
							...
							<?php foreach ($tags_group as $tg){?>
								<li class="team" style="background-color: <?php echo $tg->tg_color?>;"><a class="team-href" href="<?php echo base_url().'tags/edit/tags_group/'.$tg->tg_id?>"><?php echo $tg->tg_name?></a><a class="delete confirm" href="<?php echo base_url().'tags/delete/tags/'.$tg->tg_id?>" id="tags_group__<?php echo $tg->tg_id?>"></a></li>
							<?php }?>								
							</ul>
						<?php }?>
						</div><!-- .store-body -->
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="drag-tags-here">
								<div class="tag-store">
									<div class="store-header">
										<h6>Drag Tags Here</h6>
									</div>
									<div class="store-body">
									<?php if (!empty($tg_tags)){?>
										<ul id="dragged_tags" class="teams row dragdrop">
										<?php foreach ($tg_tags as $tg_t){?>
											<li class="team" tag_id="<?php echo $tg_t->tag_id?>" style="background-color: <?php echo $tg_t->color?>;"><a class="team-href" href="<?php echo base_url().'tags/edit/tags/'.$tg_t->tag_id?>"><?php echo $tg_t->tag_name?></a><a class="delete confirm" href="<?php echo base_url().'tags/delete/tags/'.$tg_t->tag_id?>" id="tags__<?php echo $tg_t->tag_id?>"></a></li>
										<?php }?>								
										</ul>
									<?php } else {?>
										<ul id="dragged_tags" class="teams row dragdrop">
