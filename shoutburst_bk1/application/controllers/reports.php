...
if ($text != "") {
	$wciframe = "<iframe src='http://app.shoutburst.co.uk/wordcloud_txt.php?text=$text64&logo=$logo' height=600 width=900 frameBorder='0' style='overflow:hidden;'/>";
	echo $wciframe;
} else {
		...
		{			
			...
			{				
				...
				switch ($post['filter'][$i])
				{
					...
					case 'e':
						if( $post['data_type'][$i] ==="user_pin" || $post['data_type'][$i] ==="cli" || $post['data_type'][$i] ==="dialnumb" || $post['data_type'][$i] ==="full_name")
						{
							$operator = "IN";
						}
						else
